...
int main(int argc, char** argv)
{
	...
	auto updatePrice = [&](Book& exp_book)
	{
		...
		if (exp_book.country() == "US") exp_book.price() *= usdToCadRate;
		else if ((exp_book.country() == "UK") && (exp_book.year() >= 1990) && (exp_book.year() <= 1999)) exp_book.price() *= gbpToCadRate;
	...
	for (int i = 0; i < 7; i++)updatePrice(library[i]);
		...
		if (mn_file)
		{
			...
			while (mn_count < 5)
			{
				...
				if (mn_temp[0] != '#')
				{
					...
					Movie mn_movie(mn_temp);
					movies[mn_index] = mn_movie;
					mn_count++;
					mn_index++;
	...
	else std::cout << "** No movies in the Collection\n";
	...
	catch (const std::exception& f_error)
	{

		std::cout << "** EXCEPTION: " << f_error.what() << std::endl;
	...
	if (aMovie == nullptr) std::cout << "** Movie Terminator 2 not in collection.\n";

	aMovie = theCollection["Dark Phoenix"];

	if (aMovie != nullptr) std::cout << "In this collection:\n" << *aMovie;
